...
def validate_landpoints(config):
    if config['CNTRLSCM']['landpoints'] == 1 and config['LOGIC']['land_sea_mask']:
        # print('Land or coastal case chosen')
        pass
    elif config['CNTRLSCM']['landpoints'] == 0 and not config['LOGIC']['land_sea_mask']:
...
def validate_fland_ctile(config):
    ...
    elif config['CNTRLSCM']['landpoints'] == 1 and config['RUNDATA']['fland_ctile'] > 0 and config['RUNDATA']['fland_ctile'] < 1: #TODO(jono): This will never be true
...
def validate_soil_type(config):
    if config['CNTRLSCM']['landpoints'] == 1 and config['LOGIC']['land_ice_mask'] and config['INDATA']['soil_type'] == 1:
        pass
    elif config['CNTRLSCM']['landpoints'] == 1 and config['LOGIC']['soil_mask'] and 1 < config['INDATA']['soil_type'] <= 3:
        pass
...
def validate_suface_diag(config):
    ...
    if config['CNTRLSCM']['landpoints'] == 1 and not config['DIAGS']['l_SCMDiag_sea'] and config['DIAGS']['l_SCMDiag_land']:
        # print('Land diagnostics output')
        pass
    elif config['CNTRLSCM']['landpoints'] == 0 and config['DIAGS']['l_SCMDiag_sea'] and not config['DIAGS']['l_SCMDiag_land']:
    ...
    if config['LOGIC']['obs']:
        for var in ['t_inc', 'q_star', 'u_inc', 'v_inc', 'w_inc', 't_bg', 'q_bg', 'u_bg', 'v_bg', 'w_bg']:
            if var in ['w_inc', 'w_bg']:  # w forcing has extra layer
                if len(config['INOBSFOR'][var]) != (config['CNTRLSCM']['nfor'] * (config['CNTRLSCM']['model_levels_nml'] + 1)):
                    raise Exception('Incorrect length of forcing variable {}'.format(var))
            elif len(config['INOBSFOR'][var]) != (config['CNTRLSCM']['nfor'] * config['CNTRLSCM']['model_levels_nml']):
                raise Exception('Incorrect length of forcing variable {}'.format(var))
...
def validate_intial_state(config):
    ...
    for var in ['p_in', 'thetai', 'qi', 'ui', 'vi', 'wi', 'w_advi']:
        if len(config['INOBSFOR'][var]) != (config['CNTRLSCM']['nfor'] * config['CNTRLSCM']['model_levels_nml']):
            raise Exception('Incorrect length of forcing variable {}'.format(var))
