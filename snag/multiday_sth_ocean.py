...
datetime_list = make_regular_timeseries(dt.datetime(2000, 1, 1, 13), dt.datetime(2000, 12, 29, 13), 86400)
...
for ii, i in enumerate(range(0, len(datetime_list), num_days - 1)):
    ...
    conf['data'][
        'filename'] = 'T:/DSC-SCM/scm_input/NZESM/NZESM_nudged_{}days_{}{:02}{:02}{:04}_lat{:.3f}_lon{:.3f}.nc'.format(num_days, dt1.year, dt1.month,
                                                                                                                  dt1.day, dt1.hour, lat, lon)
    ...
    conf['CNTLSCM']['nfor'] = num_days * (86400 / out_dt) + 1
    conf['RUNDATA']['ndayin'] = num_days
    conf['RUNDATA']['nminin'] = 0#(out_dt * ((86400 / out_dt) - 1)) / 60
    nc_file = nc.Dataset(conf['data']['filename'])

    conf['INPROF']['tstari'] = float(np.round(np.mean(nc_file.variables['surface_temperature'][:]),2))
    conf['RUNDATA']['tstar_sea'] = float(np.round(np.mean(nc_file.variables['surface_temperature'][:]),2))
